...
namespace TgSharp.NETCore.Tests
{
    [TestClass]
    public class TgSharpTests
    {
        ...
        public virtual async Task AuthUser()
        {
            ...
            try
            {
                user = await client.MakeAuthAsync(NumberToAuthenticate, ((TLSentCode)hash).PhoneCodeHash, code);
            }
        ...
        public virtual async Task SendMessageToChannelTest()
        {
            ...
            await client.SendMessageAsync(new TLInputPeerChannel() { ChannelId = chat.Id, AccessHash = chat.AccessHash }, "TEST MSG");
        }
        ...
        public virtual async Task DownloadFileFromWrongLocationTest()
        {
            ...
            var resFile = await client.GetFile(new TLInputPeerPhotoFileLocation()
            {
                ...
                Peer = new TLInputPeerUser { UserId = user.Id, AccessHash = user.AccessHash }
            }, 1024);
        ...
        public virtual async Task SignUpNewUser()
        {
            ...
            var registeredUser = await client.MakeAuthAsync(NotRegisteredNumberToSignUp, ((TLSentCode)sentCode).PhoneCodeHash, code, "TgSharp", "User");
        ...
        public virtual async Task SendMessageByUserNameTest()
        {
            ...
            if (user == null)
            {
                throw new ArgumentException("Username was not found: " + UserNameToSendMessage, nameof(UserNameToSendMessage));
            }
